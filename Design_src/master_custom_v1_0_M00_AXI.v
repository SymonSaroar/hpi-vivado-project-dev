...
`timescale 1 ns / 1 ps
	...
	module master_custom_v1_0_M00_AXI #
	(
...
//		parameter  C_M_TARGET_SLAVE_BASE_ADDR	= 32'h40000000,
		...
		parameter integer C_M_AXI_RUSER_WIDTH	= 8,
...
//		input wire [C_M_AXI_ADDR_WIDTH - 1 : 0] target_base_addr,
		...
		input  wire [VECTOR_FIFO_DEPTH_SIZE-1:0] vector_fifo_word_threshold,
		input  wire vector_byte_swap,
	    ...
	    output reg [TRACE_BUF_ADDR_WIDTH+$clog2(256/TRACE_BUF_DATA_WIDTH)-1:0] trace_buf_bram_addrb_debug,
		// User ports ends
		// Do not modify the ports beyond this line

		// Initiate AXI transactions
		input wire  run_program,
		input wire  program_start,
		// Stop AXI transactions
		input wire end_program,
		input wire program_error,
...
//		output reg  ERROR,
	...
	reg  	axi_wvalid;
	reg  	axi_bready;
	reg [C_M_AXI_ADDR_WIDTH-1-4 : 0] 	axi_araddr;
	reg  	axi_arvalid;
	reg  	axi_rready;
...
//    assign M_AXI_ARADDR = read_addr;
	...
	//Update value to 4'b0011 if coherent accesses to be used via the Zynq ACP port. Not Allocated, Modifiable, not Bufferable. Not Bufferable since this example is meant to test memory, not intermediate cache. 
	assign M_AXI_ARCACHE	= cache_value;
...
//	        run_program_ff <= run_program && (~(addr_fifo_empty || vector_fifo_full));
	        run_program_ff <= program_start && (~(addr_fifo_empty || vector_fifo_full));
...
//	  end                                                                                                       
	 ...
	 // against the number of burst transactions the master needs to initiate                                   
	  ...
	  // implement first_read register to indicate first read after program_start pulse / run_program_pulse

	  always @(posedge M_AXI_ACLK) begin
	  	if(M_AXI_ARESETN == 1'b0) begin
	  		first_read <= 1'b0;
	  	end else if(run_program_pulse == 1'b1) begin
	  		first_read <= 1'b1;
	  	end else if(end_program_pulse == 1'b1) begin
	  		first_read <= 1'b0;
	  	end else begin
	      ...
	      begin                                                                                                 
	        ...
	        case (mst_exec_state)                                                                               
	          ...
	          IDLE:                                                                                     
	            ...
	            if (((first_read == 1'b0 && run_program_pulse == 1'b1) || (first_read == 1'b1 && program_start == 1'b1)) && (~(addr_fifo_empty || vector_fifo_full)))
	              begin                                                                                         
...
//    assign debug_tx_num = C_TRANSACTIONS_NUM;
    ...
    wire [TRACE_BUF_ADDR_WIDTH+$clog2(256/TRACE_BUF_DATA_WIDTH)-1:0] trace_buf_bram_addrb;
    ...
    always @(posedge M_AXI_ACLK) begin
    	if(M_AXI_ARESETN == 1'b0) begin
    		...
    		vector_fifo_data_out_debug <= output_data;
    ...
    datapath_fifo #(
    	...
    	.word_threshold(vector_fifo_word_threshold),
    	.byte_swap(vector_byte_swap)
    );
    trace_buffer_bram trace_buffer (
		...
		.dina({{(256 - VECTOR_DATA_WIDTH){1'b0}}, vector_fifo_data_out}),    // input wire [255 : 0] dina
